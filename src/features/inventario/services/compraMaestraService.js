...
function calcularNecesidadSucursal({
  ...
  if (existencia < 0) {
    return {
      ...
      cantidadComprar: 0,
      coberturaDespuesCompra: null,
      inversionEstimada: 0,
      requiereRevisionInventario: true,
    };
  }
...
export async function generarCompraMaestra() {
  ...
  for (
    ...
    for (
      ...
      /*
        Los inventarios negativos siempre
        deben conservarse para revisión,
        aunque el producto no tenga ventas.

        Los productos sin demanda y con
        inventario normal no entran a
        Compra Maestra.
      */
      const requiereRevisionPorExistencia =
        convertirNumero(
          producto?.existencia
        ) < 0;
      ...
      /*
        Si la sucursal no requiere revisión
        y ya tiene suficiente inventario,
        no entra a Compra Maestra.

        Las existencias negativas sí se
        conservan para mostrarlas como
        "Revisar inventario", pero nunca
        generan compra automática.
      */
      if (
        !requiereRevisionInventario &&
        calculo
      ...
      /*
        Una sucursal con inventario negativo
        se conserva para revisión, pero NO
        modifica cantidades de compra,
        traspasos ni inversión.
      */
      if (
        !requiereRevisionInventario
      ) {
  ...
  const productosConsolidadosLista =
    Array.from(
  ...
  /*
    Lista principal:
    solamente productos que realmente
    generan compra automática.
  */
  const productos =
    productosConsolidadosLista
      .filter(
        (producto) =>
          producto.compraTotal > 0
      )
      .sort(
        (a, b) => {
          if (
  ...
  /*
    Lista independiente de revisión.

    Aquí pueden aparecer productos que
    no tengan compra automática, pero sí
    tengan una existencia negativa en
    alguna sucursal.
  */
    ...
    productosConsolidadosLista
      ...
      .sort(
        (a, b) => {
          ...
          return negativosB - negativosA;
        }
      );

  /*
    Los totales de compra se calculan
    únicamente con la lista de compra.

    Los productos de revisión NO alteran
    piezas, inversión ni necesidades.
  */
  ...
  productosConsolidadosLista.reduce(
    (total, producto) =>
      total +
      convertirNumero(
        producto
...
const totalCubiertoPorTraspasos =
  ...
  return {
    ...
    totalProductosRevisionInventario:
      productosRevisionInventario.length,
